...
import com.olp.config.auth.JWTAuthenticationFilter;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public static class JWTWebSecurityConfig extends WebSecurityConfigurerAdapter{
		...
		protected void configure(AuthenticationManagerBuilder auth) throws Exception {
			...
			auth.userDetailsService(userManagementServiceImpl).passwordEncoder(passwordEncoder());
		}
		...
		protected void configure(HttpSecurity http) throws Exception {
			...
			http.cors().configurationSource(request -> {
						...
						cors.setAllowedOrigins(List.of("http://localhost:8081","http://localhost:3000", "http://127.0.0.1:3000"));
					...
					.addFilterBefore(new JWTAuthenticationFilter(userManagementServiceImpl, jwtTokenHelper), UsernamePasswordAuthenticationFilter.class);
